...
def gen_table_fields_infos_from_dataframe(frame):
    ...
    columns_types = [str(x) for x in list(frame.dtypes)]

    index_type = frame.index.inferred_type

    res = list(zip(columns, columns_types))
...
# 获取一个数据库中所有table的列表
#------------------------------------------------------------------------------
def get_all_tables_in_database(database_name):
...
# 获取一张表中最后一行的时间戳
#------------------------------------------------------------------------------
def get_last_row_timestamp(client, database_name, table_name):
    select_query = "select timestamp from " + database_name + "." + table_name
    res = client.execute(select_query)
